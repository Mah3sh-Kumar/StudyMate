...
export const mockUsers = [
  ...
  {
    id: "user-4",
    email: "krishnarai4657@gmail.com",
    password: "123",
    user_metadata: {
    ...
    profile: {
      ...
      username: "krishnarai",
      email: "krishnarai4657@gmail.com",
      avatar_url: "https://i.pravatar.cc/150?u=krishnarai4657@gmail.com",
      study_subjects: ["Computer Science", "Mathematics", "Technology"],
      study_goals: ["Master programming", "Learn algorithms", "Build projects"],
      preferences: {
        study_duration: 30,
        break_duration: 5,
        daily_goal_hours: 5
...
export const mockFlashcardDecks = [
  ...
  },
  {
    ...
    description: "Essential programming concepts and algorithms",
    subject: "Computer Science",
    is_public: false,
    total_cards: 12,
    created_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
    tags: ["programming", "algorithms", "coding", "fundamentals"]
  },
  {
    id: "deck-5",
    user_id: "user-4", 
    name: "Data Structures",
    description: "Important data structures and their applications",
    subject: "Computer Science",
    is_public: true,
    total_cards: 8,
    created_at: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
    updated_at: new Date(Date.now() - 12 * 60 * 60 * 1000).toISOString(),
    tags: ["data-structures", "arrays", "trees", "graphs"]
  }
];
...
export const mockFlashcards = [
  ...
  // Programming Fundamentals deck cards (user-4)
  {
    id: "card-7",
    deck_id: "deck-4",
    front: "What is Big O notation?",
    back: "Big O notation describes the upper bound of algorithm time complexity, representing worst-case performance as input size grows.",
    difficulty_level: 2,
    next_review: null,
    review_count: 0,
    created_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  },
  {
    ...
    back: "Recursion is a programming technique where a function calls itself to solve a smaller version of the same problem.",
    difficulty_level: 2,
    next_review: null,
    review_count: 1,
  ...
  },
  {
    ...
    back: "== performs type coercion and compares values, while === compares both value and type without coercion.",
    difficulty_level: 1,
    next_review: null,
    review_count: 3,
    created_at: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  // Data Structures deck cards (user-4)
  {
    id: "card-10",
    deck_id: "deck-5",
    front: "What is a binary search tree?",
    back: "A binary search tree is a tree data structure where each node has at most two children, and for each node, left children are smaller and right children are larger.",
    difficulty_level: 3,
    next_review: null,
    review_count: 0,
    created_at: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  },
  {
    id: "card-11",
    deck_id: "deck-5",
    front: "What is the time complexity of array insertion at the beginning?",
    back: "O(n) because all existing elements need to be shifted one position to the right.",
    difficulty_level: 2,
    next_review: null,
    review_count: 2,
    created_at: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(),
...
export const userService = {
  ...
  async getProfile(userId) {
    try {
      ...
      if (!user) {
        ...
        logOperation('SELECT', 'user_profile', result, { userId });
        return result;
      }
      
      const result = { data: user.profile, error: null };
      logOperation('SELECT', 'user_profile', result, { userId });
  ...
  async updateProfile(userId, updates) {
    try {
      ...
      if (userIndex === -1) {
        const result = { data: null, error: { message: 'User not found' } };
        logOperation('UPDATE', 'user_profile', result, { userId, updates });
        return result;
      }
      ...
      if (updates.username) {
        user.profile.username = updates.username;
        user.user_metadata.username = updates.username;
      }
      if (updates.email) {
        user.profile.email = updates.email;
        user.email = updates.email;
      }
      if (updates.study_subjects) {
        user.profile.study_subjects = updates.study_subjects;
      }
      if (updates.study_goals) {
        user.profile.study_goals = updates.study_goals;
      }
...
export const flashcardService = {
  ...
  async getUserDecks(userId) {
    try {
      console.log('🃏 Fetching user flashcard decks...', { 
        userId, 
        totalDecks: mockFlashcardDecks.length,
        allDeckUsers: mockFlashcardDecks.map(d => d.user_id)
      });
      
      const decks = getUserFlashcardDecks(userId);
      
      console.log('🃏 Found decks for user:', {
        userId,
